...
{
    if (numItems == capacity) { //if array reaches capacity then expand
        expand();
    }
...
bool ArrayList::remove(string item)
{
    ...
    if (numItems > DEFAULT_SIZE && numItems < (capacity / 2)) { // if capacity is 2x numItems & there are more items than the default size then shrink
        shrink();
    }
 ...
 * Parameters: int newSize - the desired capacity of the list. The array will
 *             be updated to have space for newSize elements.
 *    Returns: nothing
 * 
 *       Note: If there are any elements past index newSize, they will be lost.
 */
void ArrayList::resize(int newSize)
{
    ...
    delete [] arr; // delete old pointer contents
    arr = tempArray; // replace arr with tempArr pointer
}
